...
export async function bytecodeCall(params: ByteCodeCallOptions): Promise<any> {
  ...
  if (chain === "tron")
    throw new Error("Bytecode call is not supported on Tron");
...
export async function multiCall(params: MulticallOptions): Promise<any> {
  ...
  if (params.target && !isValidTarget(params.target, chain))
    throw new Error("Invalid target: " + params.target);
  ...
  if (!params.target && !params.permitFailure) {
    ...
    if (
      params.calls.some((i: any) => {
        if (typeof i === "string" && i.startsWith("0x"))
          return !isValidTarget(i, params.chain);
        if (typeof i === "object")
          return !isValidTarget(i.target, params.chain);
        return true;
      })
    ) {
  ...
  const results = await runInPromisePool({
    ...
    concurrency: 10,
    processor: (calls: any) =>
      makeMultiCall(abi, calls, chain as Chain, params.block),
  });
  ...
  params.contractCalls.forEach((value: any, i: number) => {
    if (!isValidTarget(value.contract, params.chain)) {
      ...
      debugLog(
        ...
        "Bad target passed:",
        value.contract,
        "skipping call and returning null output for this",
      );
      return;
    }
...
const cachableAbiSet = new Set(["uint8:decimals"]);

function isCachableAbi(abi: string): boolean {
  if (typeof abi !== "string") return false;
  return (
    abi.startsWith("address:") ||
    abi.startsWith("string:") ||
    cachableAbiSet.has(abi)
  );
}
