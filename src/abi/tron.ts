...
export async function call(options: CallOptions) {
  ...
  params.forEach((v: any, i: number) => {
    if (inputTypes[i] === "address" && !v.startsWith("0x"))
      params[i] = toHex(v);
  });
  ...
  const body = {
    ...
    function_selector: (
      contractInterface.fragments[0] as ethers.FunctionFragment
    ).format(),
...
export async function getBalance(params: {
  ...
  const data = await limitRPCCalls(() =>
    post({ address: params.target, visible: true }, "/wallet/getaccount"),
  );
  const balance = (
    (data.balance ?? 0) +
    (data.frozen?.reduce(
      (t: any, { frozen_balance }: any) => t + frozen_balance,
      0,
    ) ?? 0)
  ).toString();
...
export async function getBalances(params: {
  ...
  const res = await Promise.all(
    targets.map((i) => getBalance({ target: i, decimals })),
  );
  return {
    output: res.map((v: any, i: number) => ({
      target: targets[i],
      balance: v.output,
    })),
  };
...
async function executeCalls(
  ...
  return runInPromisePool({
    ...
    processor: async ({ contract, params }: any) => {
      let result = null;
      try {
        result = (await call({ target: contract, params, abi: functionABI }))
          .output;
      } catch (e) {
        debugLog(e);
      }
