...
test("ChainApi - cosmos ", async () => {
  ...
  expect(api.getBalances()).toEqual({
    "ethereum:0x1Da189c1BA3d718Cc431a2ed240a3753f89CD47A": 5,
    "injective:injectiveNative": 1,
    "ibc:kijura:isk123": 1,
    "ibc:kijura:Factory:123": 5,
  });
});
test("ChainApi - log", async () => {
  const api = new ChainApi({});
...
test("bad paramters throw error", async () => {
  ...
  await apiBsc.multiCall({
    abi: testAbi,
    calls: ["0xa098751d407796d773032f5cc219c3e6889fb893"],
  });
  ...
  await expect(
    apiEth.multiCall({ abi: testAbi, calls: [{}] }),
  ).rejects.toThrowError();
  await expect(
    apiEth.multiCall({ abi: testAbi, calls: [nullAddress] }),
  ).rejects.toThrowError();
});
...
test("bad paramters does not throw error with permitFailure Flag", async () => {
  ...
  const apiBsc = new ChainApi({ chain: "bsc" });
  const apiEth = new ChainApi({});
  const testRes = await apiEth.multiCall({
    abi: testAbi,
    permitFailure: true,
    calls: [nullAddress, "ethTest"],
  });
  const testRes2 = await apiBsc.multiCall({
    abi: testAbi,
    permitFailure: true,
    calls: [
      nullAddress,
...
test("ChainApi - sumTokens - use tokens", async () => {
  ...
  expect(
    +res["ethereum:0xdac17f958d2ee523a2206206994597c13d831ec7"],
  ).toBeGreaterThan(0);
});
...
test("ChainApi - sumTokens - use tokensAndOwners", async () => {
  ...
  const res = await api.sumTokens({
    tokensAndOwners: [
      [
        ...
        "0x9a849A108764a5cE2Ab4CD3208071B304a9Ac99A",
      ],
      [
        "0x419B8ED155180A8c9C64145e76DaD49c0A4Efb97",
        "0x9a849A108764a5cE2Ab4CD3208071B304a9Ac99A",
  ...
  const key1 =
    "ethereum:0x419B8ED155180A8c9C64145e76DaD49c0A4Efb97".toLowerCase();
...
test("ChainApi - sumTokens - use ownerTokens", async () => {
  ...
  const res = await api.sumTokens({
    ownerTokens: [
      [
        ["0x419B8ED155180A8c9C64145e76DaD49c0A4Efb97", nullAddress],
        "0x9a849A108764a5cE2Ab4CD3208071B304a9Ac99A",
      ],
    ],
  });
  ...
  const res = await api.sumTokens({
    tokensAndOwners2: [
      [
        ...
        "TNUC9Qb1rRpS5CbWLmNMxXBjyFoydXjWFR",
        "TNUC9Qb1rRpS5CbWLmNMxXBjyFoydXjWFR",
        "TKqvrVG7a2zJvQ3VysLoiz9ijuMNDehwy7",
      ],
      [
        "TS2xnL5XW4JqMgSVizrhGLKNBekn5ubrLr",
        "TLw6HAYJxZG2SEsmn2fx8myaqeFkGRa9KH",
        "TAUtMLMQUcabAr48pJgxNUmX2zfDMSPptb",
        "TEYbSjBNBN1kphjvbjKsBbtPLa5QdgArdx",
        "TXPbj1xgKKWFuM6R4qG9iQx1Ao1aiBShML",
        "TVGdFgp1oKksGXcozoYdM614by7pdJnzty",
      ],
    ],
  });
...
test("ChainApi - sumTokens - duplicated entries", async () => {
  ...
  expect(
    +res["ethereum:0x0000000000000000000000000000000000000000"],
  ).toBeLessThan(3 * 1e23 * 1.1);
  expect(
    +res["ethereum:0x0000000000000000000000000000000000000000"],
  ).toBeGreaterThan(3 * 1e23);
});
...
test("ChainApi - sumTokens - era", async () => {
  ...
  expect(
    +res["era:0x0000000000000000000000000000000000000000"],
  ).toBeGreaterThan(0);
});
...
test("ChainApi - erc4626Sum - 1", async () => {
  ...
  await api.erc4626Sum({
    calls: balancerPools,
    tokenAbi: "address:stakingToken",
    balanceAbi: "uint256:totalDeposits",
  });
...
test("ChainApi - erc4626Sum - 2", async () => {
  ...
  await api.erc4626Sum({
    calls: balancerPools,
    tokenAbi: "stakingToken",
    balanceAbi: "totalDeposits",
  });
  expect(Object.keys(api.getBalances()).length).toBeGreaterThan(0);
});
...
test("ChainApi - erc4626Sum - 3", async () => {
  ...
  await api.erc4626Sum({
    calls: ["0x0DB2BA00bCcf4F5e20b950bF954CAdF768D158Aa"],
    isOG4626: true,
  });
});
