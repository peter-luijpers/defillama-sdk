import {
  ...
  getChainRPCs,
  getDefaultChunkSize,
  getEnvMulticallAddress,
  getEnvCacheFolder,
  getEnvRPC,
  getEnvValue,
  getMaxParallelRequests,
  getParallelGetLogsLimit,
} from "./env";
...
test("cronos calls over batch limit", async () => {
  ...
  const calls = [] as any;
  for (let i = 0; i < limit + 1; i++) calls.push(call);

  const cronosApi = new ChainApi({ chain: "cronos" });
  await Promise.all(
    calls.map((call: any) =>
      cronosApi.call({ target: call, abi: "string:name" }),
    ),
  );
});
...
test("getArchivalRPCs", async () => {
  ...
  process.env["SDK_TESTCHAIN_ARCHIVAL_RPC"] =
    "http://localhost:8545,http://localhost:8546";
  const archivalRPCsAfter = getArchivalRPCs("testchain");
  expect(archivalRPCsAfter).toEqual([
    "http://localhost:8545",
    "http://localhost:8546",
  ]); // value from process.env
