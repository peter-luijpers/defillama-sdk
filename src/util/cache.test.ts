...
test("Cache - parseCache & compressCache", async () => {
  ...
  expect((await parseCache(compressedData.toString("base64"))).test).toEqual(
    inputData.test,
  );
...
test("Cache - write string", async () => {
  ...
  const stringData =
    "this is a string; Lopum ipsum dolor sit amet, consectetur adipiscing elit. Nulla euismod, nisl eget aliquam ultricies, nisl nisl aliquet nisl, nec aliquam nisl nisl nec.";
