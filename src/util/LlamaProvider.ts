...
export class LlamaProvider extends FallbackProvider {
  ...
  async _ready() {
    ...
    Object.entries(currentBlocks).forEach(([url, block]) => {
      if (medianBlockValue - block > outOfSyncLimit) {
        debugLog(
          `RPC ${url} is out of sync by ${medianBlockValue - block} blocks`,
        );
  ...
  async call(_tx: TransactionRequest): Promise<string> {
    return this._performAction("call", [
      { to: _tx.to, data: _tx.data },
      _tx.blockTag ?? "latest",
    ]);
  }
  ...
  async _performAction(
    ...
    if (method === "getLogs") {
      ...
      this.rpcs
        .filter((i) => !runners.map((i) => i.url).includes(i.url))
        .forEach((i) => runners.push(i)); // ensure that there are no duplicates
      runners = runners.filter((i) => !i.url.includes("llamarpc.com"));
    }
    let primaryRunner = runners[0];
    if (!primaryRunner)
      throw new Error("No RPCs available for " + this.chainName);
    let errors = [];
    runners = runners.slice(1).sort(() => Math.random() - 0.5); // randomize order of runners
    const isArchivalRequest =
      ["call", "getBalance"].includes(method) && params[1] !== "latest";
    let noPlayingAround =
      getEnvValue("RPC_NO_PLAYING_AROUND") === "true" ||
      primaryRunner.url.includes("llama.fi") ||
      method === "getLogs" ||
      isArchivalRequest;
...
function createProvider(
  ...
  try {
    ...
    const getProviderWithURL = (url: string) => ({
      url,
      provider: getProviderObject(url, name),
    });
    ...
    return new LlamaProvider(
      { providers, archivalRPCs, chainId, chainName: name },
      fallbackConfig,
    );
    ...
    if ((providerList as any)[name]) throw e;
    return null;
  }
...
export function setProvider(chain: Chain, provider: AbstractProvider) {
  providers[chain] = provider;
}

export function getProvider(
  chain: Chain = "ethereum",
  _getArchivalNode = false,
): AbstractProvider {
  if (providers[chain]) return providers[chain];

  // use RPC from env variable if set else use RPC from providers.json
  let rpcList: string | undefined = getChainRPCs(
    chain,
    (providerList as any)[chain]?.rpc,
  );
  ...
  if (!providers[chain])
    providers[chain] = createProvider(
      chain,
      rpcList,
      (providerList as any)[chain]?.chainId,
      archivalRPCList,
    ) as any;
