const _ENV_CONSTANTS = {
  DEBUG_ENABLED:
    getEnvValue("DEBUG") === "true" || process.env.LLAMA_DEBUG_MODE
      ? "true"
      : "false",
} as {
...
whitelistedEnvConstants.forEach(
  (key) => (_ENV_CONSTANTS[key] = getEnvValue(key)),
);
Object.keys(defaultEnvValues).forEach(
  (key) => (_ENV_CONSTANTS[key] = getEnvValue(key, defaultEnvValues[key])),
);
...
export function getParallelGetLogsLimit(chain: string, providerUrl?: string) {
  ...
  const defaultAlchemyLimit = getEnvValue(
    "ALCHEMY_GET_LOGS_CONCURRENCY_LIMIT",
    "25",
  );
  if (providerUrl && providerUrl.includes("alchemy.com"))
    return +defaultAlchemyLimit!;
...
export function getParallelGetBlocksLimit(chain: string) {
  ...
  return +getEnvValue(
    `${chain}_RPC_GET_BLOCKS_CONCURRENCY_LIMIT`,
    defaultLimit,
  )!;
}
...
export function getEnvValue(key: string, defaultValue?: string) {
  key = key.toUpperCase();
  return (
    process.env["LLAMA_SDK_" + key] ??
    process.env["SDK_" + key] ??
    process.env[key] ??
    defaultValue
  );
}
...
export function getBatchMaxCount(chain: string): number {
  ...
  switch (chain) {
    case "zkfair":
      return 1;
    default:
      return +getEnvValue("BATCH_MAX_COUNT", "5")!;
  }
}
