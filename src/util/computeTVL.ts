...
export default async function computeTVL(
  balances: Balances,
  timestamp?: number,
) {
...
async function updatePriceCache(keys: string[], timestamp?: number) {
  ...
  if (
    !timestamp &&
    lastPriceUpdate &&
    +Date.now() > lastPriceUpdate + priceUpdateTime
  ) {
  ...
  for (const chunk of chunks) {
    ...
    chunk
      .map((i) => i.toLowerCase())
      .filter((i) => !pricesCache[i])
      .forEach((i) => (pricesCache[i] = {}));
  }
  ...
  async function getPrices(keys: string[]) {
    ...
    const { coins } = await axios
      .post(
        "https://coins.llama.fi/prices?source=internal&apikey=" + coinsApiKey,
        { coins: keys, timestamp },
      )
      .then((res) => res.data);
    return coins;
  }
