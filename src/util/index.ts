...
export async function getLogs(
  ...
  const getLimiter = () =>
    getChainLimiter(options.chain!, getProviderUrl(options.provider));
  ...
  if (!options.provider) {
    ...
    try {
      ...
      debugLog(
        `Error fetching logs for chain ${options.chain} ${formError(e, { chain: options.chain, target: options.target })}, retrying...`,
      );
...
async function _getLogs(
  ...
  while (currentBlock < params.toBlock) {
    ...
    try {
      ...
      debugLog(
        `Error fetching logs for chain ${params.chain} blockSpread: ${blockSpread}. ${formError(e, { chain: params.chain, target: params.target, provider })}`,
      );
      if (blockSpread >= 1500) {
