...
export async function getChainBlocks(
  ...
  const setBlock = async (chain: Chain) =>
    (chainBlocks[chain] = await getBlockNumber(chain, timestamp));
...
export async function getBlocks(
  timestamp: number,
  chains: Chain[] | undefined = undefined,
) {
  chains = chains?.filter((i) => i !== "ethereum");
  const [ethBlock, chainBlocks] = await Promise.all([
    getBlockNumber("ethereum", timestamp),
    getChainBlocks(timestamp, chains),
  ]);
...
export async function getCurrentBlocks(
  ...
  if (chains) chains = chains.filter((i) => i !== "ethereum");
...
export async function getBlock(
  ...
  if (!timestamp) return getCurrentChainBlock(chain);
...
export function getCurrentChainBlock(
...
function getExtraProvider(chain: string | undefined) {
  if (chain && isCosmosChain(chain)) return getCosmosProvider(chain);
  if (chain === "algorand") return algorandBlockProvider;
...
function validateCurrentBlock(block: Block, chain: Chain = "ethereum") {
  ...
  if (minutesDiff > 60)
    throw new Error(
      `Last block for ${chain} is ${minutesDiff} minutes behind (${new Date(block.timestamp * 1000)}). Provider is "${getProviderUrl(provider)}"`,
    );
}
...
async function _lookupBlock(
  ...
  const { allowedTimeRange = 5 * 60, acceptableBlockImprecision = 10 } =
    extraParams ?? {};
  ...
  if (blockscoutEndpoints[chain]) {
    try {
      ...
      // fallback to usual way of fetching logs
  ...
  try {
    ...
    if (["evmos"].includes(chain)) {
      ...
      switch (chain) {
        default:
          firstBlockNum -= 2 * 1e5; // evmos hold only the last 200k block data
      }
    ...
    if (!highBlock) highBlock = lastBlock;
    ...
    while (
      imprecision! > allowedTimeRange &&
      blockImprecision! > acceptableBlockImprecision
    ) {
      // We lose some precision (max ~15 minutes) but reduce #calls needed
      ++i;
      ...
      let closeBlock = Math.floor(
        lowBlock.number + (timestamp - lowBlock.timestamp) / avgBlockTime,
      );
    ...
    debugLog(
      `chain: ${chain} block: ${block!.number} #calls: ${i} imprecision: ${Number(imprecision! / 60).toFixed(2)} (min) Time Taken: ${Number((Date.now() - time) / 1000).toFixed(2)} (in sec)`,
    );
  ...
  function updateBlock(blocks: TimestampBlock[] = []) {
    ...
    const getPrecision = (block: TimestampBlock) =>
      block.timestamp - timestamp > 0
    ...
    lowBlock = blocks
      ...
      .reduce((lowestBlock, block) =>
        timestamp - lowestBlock.timestamp < timestamp - block.timestamp
          ? lowestBlock
          : block,
      );
    highBlock = blocks
      .filter((i) => i.timestamp > timestamp)
      .reduce((highestBlock, block) =>
        highestBlock.timestamp - timestamp < block.timestamp - timestamp
          ? highestBlock
          : block,
      );
  ...
  function getChainBlockTimeCache(chain: string) {
    if (!blockTimeCache[chain]) blockTimeCache[chain] = {};
