...
function unzipAsync(data: Buffer) {
  return promisify(brotliDecompress)(data, brotliOptions).then((r) =>
    r.toString(),
  );
}
...
export async function writeCache(
  ...
  try {
    const fileData = options.alreadyCompressed
      ? data
      : await compressCache(data);

    if (
      !data ||
      (typeof data === "string" && data.length < 20) ||
      fileData.length < 20
    ) {
...
export async function compressCache(data: any) {
  const comressedCache = await zipAsync(
    JSON.stringify({ version: currentVersion, llamaWrapped: data }),
  );
...
export function getTempLocalCache({
  file,
  defaultData = {},
  clearAfter = ONE_WEEK,
}: {
  ...
  try {
    ...
    if (now - fileData.created > clearAfter) fileData = emptyFile; // clear cache
  } catch (error) {}
  ...
  function saveCache() {
    ...
    try {
      ...
      debugLog("Error saving cache:", error);
    }
    process.exit(0);
  }
...
export async function writeExpiringJsonCache(
  file: string,
  data: any,
  {
    expireAfter = 60 * 60 * 24, // cache for 1 day by default
    expiryTimestamp,
  }: { expireAfter?: number; expiryTimestamp?: number },
): Promise<void> {
  file = "expiring/" + file;
  if (!expiryTimestamp)
    expiryTimestamp = Math.floor(Date.now() / 1e3) + expireAfter;
