...
async function main() {
  ...
  Object.values(providerList).forEach(
    (i: any) => (providerIDMap[i.chainId] = i.rpc),
  );
  chainData = chainData
  ...
  for (const i of chainData) {
    i.rpc = await filterForWorkingRPCs(
      i.rpc.map((j: any) => j.url),
      i.name,
      i.chainId,
    );
    ...
    if (providerIDMap[i.chainId]) {
      ...
      debugLog(
        `Duplicate short name ${i.chainId} for ${i.shortName}, doing nothing`,
      );
    } else {
...
async function filterForWorkingRPCs(
  rpc: string[],
  chain: string,
  chainId: number,
): Promise<string[]> {
