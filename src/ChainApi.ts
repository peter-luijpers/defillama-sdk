...
export class ChainApi {
  ...
  async getBlock(): Promise<number> {
    if (!this.block)
      this.block = await getBlockNumber(this.chain as Chain, this.timestamp);
  ...
  async erc4626Sum({
    calls,
    tokenAbi = "address:token",
    balanceAbi = "uint256:balance",
    balanceCalls,
    permitFailure = false,
    isOG4626 = false,
  }: {
    ...
    if (
      typeof tokenAbi === "string" &&
      !tokenAbi.includes(":") &&
      !tokenAbi.includes("(")
    )
      tokenAbi = `address:${tokenAbi}`;
    if (
      typeof balanceAbi === "string" &&
      !balanceAbi.includes(":") &&
      !balanceAbi.includes("(")
    )
      balanceAbi = `uint256:${balanceAbi}`;

    const tokens = await this.multiCall({
      calls,
      abi: tokenAbi,
      permitFailure,
    });
    const balances = await this.multiCall({
      calls: calls ?? balanceCalls,
      abi: balanceAbi,
      permitFailure,
    });
  ...
  async sumTokens({
    ...
    if (tokensAndOwners2.length)
      tokensAndOwners.push(
        ...tokensAndOwners2[0].map((i: string, j: number) => [
          i,
          tokensAndOwners2[1][j],
        ]),
      );
    ...
    if (tokens.length) {
      ...
      tokensAndOwners.push(
        ...tokens.map((i) => owners.map((j) => [i, j])).flat(),
      );
    }
    ...
    tokensAndOwners = getUniqueTokensAndOwners(
      tokensAndOwners,
      this.chain as string,
    );
    ...
    tokensAndOwners = tokensAndOwners.filter(
      (i) =>
        !blacklistedTokens.includes(i[0]) && !blacklistedOwners.includes(i[1]),
    );
    const ethBalOwners = tokensAndOwners
      .filter((i) => i[0] === nullAddress)
      .map((i) => i[1]);
    ...
    if (ethBalOwners.length) {
      ...
      if (multicallAddress) {
        ...
        const res = await getBalances({
          ...
          targets: ethBalOwners,
          block: this.block as number,
        });
