...
export function convertToBigInt(value: any) {
  ...
  if (typeof value === "string") {
    if (!value.startsWith("0x") && value.includes("e")) {
      ...
      if (exponent >= 0) {
        final =
          lead +
          (decimal || "") +
          "0".repeat(exponent - (decimal || "").length);
      } else {
...
export function sumMultiBalanceOf(
  ...
  results.output.map((result) => {
    if (result.success) {
      ...
      balances[address] = (
        BigInt(balances[address] ?? 0) + BigInt(balance)
      ).toString();
...
export function sumSingleBalance(
  ...
  if (typeof balance === "object") {
    if (typeof balance.toString === "function") balance = balance.toString();
    else throw new Error("Invalid balance value:" + balance);
  }

  if (
    typeof balance === "number" ||
    (balances[token] && typeof balances[token] === "number")
  ) {
    const prevBalance = +(balances.hasOwnProperty(token) ? balances[token] : 0);
    if (typeof prevBalance !== "number" || isNaN(prevBalance))
      throw new Error(
        `Trying to merge token balance and coingecko amount for ${token} current balance: ${balance} previous balance: ${balances[token]}`,
      );
...
export function removeTokenBalance(
  balances: Balances,
  token: string,
  ...
  return async (
    ...
    await Promise.all(
      chainTvls.map(async (chainTvl) => {
        const chainBalances = await chainTvl(
          timestamp,
          ethBlock,
          chainBlocks,
          params,
...
export function getProviderUrl(provider: any) {
  if (provider?.isCustomLlamaProvider)
    return provider.rpcs.map((i: any) => i.url).join(", ");
  if (provider instanceof ethers.FallbackProvider)
    provider = provider.providerConfigs[0]
      .provider as ethers.JsonRpcApiProvider;
...
export function formErrorString(e: any, errorParams: any = {}) {
  if (!e) {
    if (errorParams.promisePoolErrors) {
      const errors = errorParams.promisePoolErrors.map((i: any) =>
        formErrorString(i),
      );
      return `Promise pool failed! \n ${errors.join("\n")}`;
    }
    if (errorParams.isMultiCallError) {
      const targetStr = errorParams.target
        ? `[target: ${errorParams.target}]`
        : "";
      ...
      errorParams.failedQueries?.forEach((i: any) => {
        ...
        let params =
          i.params && i.params.length
            ? `params: ${i.params.length === 1 ? i.params[0] : i.params.join(", ")}]`
            : "";
        errorString += `\n ${target} ${params}`;
      });
      // truncate error string to 310 chars
      return errorString.length > 310
        ? errorString.slice(0, 310).concat("...")
        : errorString;
    }
    return "";
  }
  ...
  if (e.llamaRPCError) {
    const errorString = `Llama RPC error! methos: ${e.method} `;
    const errors = e.errors.map(
      (i: any) => `- host: ${i?.host} error: ${i?.error?.message}`,
    );
    return errorString + "\n" + errors.join("\n");
  }
  ...
  if (
    ((e.reason || e.method || e.shortMessage) && e.code) ||
    errorParams.result === "0x"
  ) {
    // ethers.js error
    ...
    } else if (!e.provider && errorParams.provider)
      e.provider = errorParams.provider;

    let providerUrl = getProviderUrl(e.provider ?? errorParams.provider);
    if (e.serverError)
      return `host: ${providerUrl} ${e.serverError.toString()}`;

    if (e.results) {
      let errors = e.results
      ...
      if (
        errorParams.params &&
        Array.isArray(errorParams.params) &&
        errorParams.params.length
      )
        extraInfo += shortenString(" params: " + errorParams.params.join(", "));
    ...
    if (method === "eth_blockNumber")
      return `host: ${providerUrl} reason: ${e.reason} code: ${e.code}`;
    if (method === "getBlockNumber")
      return `Failed to call ${method} ${providerUrl} reason: ${e.reason} code: ${e.code}`;
...
export function formError(e: any, errorParams: any = {}): Error {
  ...
  if (
    e?._isCustomError ||
    (!errorParamsSet && typeof e === "object" && !Object.keys(e).length)
  )
    return e; // already formatted or vannila error
  const error: any = new Error(formErrorString(e, errorParams));
  try {
    if (errorParams.isMultiCallError)
      error._underlyingErrors = errorParams?.failedQueries?.map((i: any) =>
        shortenString(i.error.toString()),
      );
    else error._underlyingError = shortenString(e.toString());
  } catch (e) {}
