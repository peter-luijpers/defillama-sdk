...
export class Balances {
  ...
  constructor(params: { chain?: Chain | string; timestamp?: number }) {
  ...
  addTokens(tokens: string[], balances: any[], { skipChain = false } = {}) {
    ...
    if (tokens.length !== balances.length)
      throw new Error("token and balance must have the same length");
  ...
  async getUSDJSONs() {
    const { usdTvl, usdTokenBalances } = await computeTVL(
      this.getBalances(),
      this.timestamp,
    );
