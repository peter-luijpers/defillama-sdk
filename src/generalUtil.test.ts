...
test("sumSingleBalance throw error on invalid input", () => {
  expect(() =>
    sumSingleBalance({ ethereum: 1 }, "dummy", { bad: 1 } as any),
  ).toThrowError();
  expect(() =>
    sumSingleBalance({ ethereum: 1 }, "dummy", null as any, "bsc"),
  ).toThrowError();
  expect(() =>
    sumSingleBalance({ ethereum: "1" }, "ethereum", undefined as any),
  ).toThrowError();
  expect(() =>
    sumSingleBalance({ ethereum: 1 }, "dummy", "a111" as any, "ethereum"),
  ).toThrowError();
  expect(() =>
    sumSingleBalance({ ethereum: "1" }, "dummy", "111a" as any, "ethereum"),
  ).toThrowError();
});

test("sumSingleBalance with numbers", () => {
  const balances: any = {};
  const veryBigNumber = 1.731174581703269e21;
...
test("convertToBigInt", () => {
  ...
  expect(convertToBigInt(2.031945223e22)).toBe(
    BigInt("20319452230000000000000"),
  );
  expect(convertToBigInt("2.031e+22")).toBe(BigInt("20310000000000000000000"));
  expect(convertToBigInt("0x0000000000000000000139C5FfeE6153a7b8678f")).toBe(
    BigInt("1481753159505255786375055"),
  );
});
